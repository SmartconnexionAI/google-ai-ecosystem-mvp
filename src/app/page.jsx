...
const tools = [
  ...
  // Create with AI
  { name: "Gemini 3 Flash Image (Nano Banana 2)", category: "Create with AI", description: "Image generation and editing", tags: ["image"] },
...
export default function Page() {
  ...
  const tags = [
    "All","model","agent","video","api","cloud","builder","models","chat","search",
    "image","vision","writing","data","presentation","email","meeting","coding",
    "learning","audio","design"
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f172a] to-[#020617] text-white p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {categories.map(c => (
            <button key={c} onClick={()=>setFilter(c)} className={`px-4 py-2 rounded-xl border ${filter===c?"bg-cyan-500 text-black":"bg-slate-800"}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        {filter !== "What's Changed" && (
          <div className="flex flex-wrap gap-2 mb-6">
            {tags.map(t => (
              <button key={t} onClick={()=>setTag(t)} className={`px-3 py-1 rounded-full text-sm ${tag===t?"bg-purple-500":"bg-slate-800"}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        {filter !== "What's Changed" && (
          <div className="grid md:grid-cols-2 gap-4">
            {filteredTools.map(t => (
              <div key={t.name} onClick={()=>setSelectedTool(t)} className="p-4 rounded-xl bg-slate-900 border border-slate-700 cursor-pointer">
        ...
        <div className="mt-10">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {goals.map(g => (
              <button key={g.name} onClick={()=>setSelectedGoal(g.name)} className={`px-4 py-2 rounded-xl ${selectedGoal===g.name?"bg-purple-500":"bg-slate-800"}`}>
                {g.name}
              </button>
            ))}
          </div>
            ...
            <div className="bg-slate-900 p-6 rounded-xl max-w-lg w-full">
              ...
              {(prompts[selectedTool.name]||[]).map((p,i)=> (
                <div key={i} className="bg-slate-800 p-3 mb-2 rounded">
                  ...
                  <button onClick={()=>navigator.clipboard.writeText(p)} className="text-xs bg-purple-500 px-2 py-1 rounded">Copy</button>
                </div>
              ))}
              <button onClick={()=>setSelectedTool(null)} className="mt-4 bg-cyan-500 text-black px-4 py-2 rounded">Close</button>
            </div>
          </div>
        )}
